	...
	const getGPSForAddress = function(scheme, arr, address){
		...
		return (new Promise(resolve => {
			...
			const compareResult = (addr1, addr2, obj) => {
				...
				return (new Promise(resolve => {
					...
					const getData = function * (){
						...
						if($.isset(addr1) && $.isString(addr1)
						&& $.isset(addr2) && $.isString(addr2)){
							...
							// Если сравнение не удалось то сообщаем что не удачно
								...
								for(let subject in newObj){
									...
									else if($.isset(newObj[subject])
									&& $.isString(newObj[subject])) key = subject;
								}
			...
			const getData = function * (arr, i){
				...
				&& ($.isset(arr[i].name) && $.isString(arr[i].name)
				&& $.isset(arr[i].type) && $.isString(arr[i].type))){
	...
	class Agl {
		...
		getVersionSystem(){
			...
			return (new Promise(resolve => {
				...
				idObj.log(
					"\x1B[31m\x1B[1m"
					.anyks_clearColor(idObj.debug.color),
					"v" + idObj.version,
					"\u00A9", idObj.copyright,
					"\x1B[0m".anyks_clearColor(idObj.debug.color)
				).info();
				// Выводим результат
				resolve(object);
			}));
			...
			return (new Promise(resolve => {
				...
				const reg = [{
					...
					// Дома
						...
						"type":	"house",
						"reg":	new RegExp("(?:(?:№\\s*)?\\d+[А-ЯЁ\\_]*\\s*(?:\\/|-)\\s*\\d+[А-ЯЁ\\_]*)|"
								+ "(?:(?:№\\s*)?(?:\\d+)[А-ЯЁ\\_]*\\s*[А-ЯЁ\\_]*\\s*(?:\\d+)?\\s*[А-ЯЁ\\_]*\\s*(?:\\d+)?)", "i")
					// Реки
					},{
						"type":	"river",
				...
				Agl.getRedis.call(idObj, "get", key, 3600).then(({err, cache}) => {
					...
					// Если данные в кеше не найдены тогда продолжаем искать
						...
						const getData = function * (){
							...
							// Если это массив то извлекаем данные
							if($.isArray(region) && region.length) region = yield compareGPS.call(idObj, lat, lng, region);
							// Создаем объект для сохранения данных
				...
				Agl.getRedis.call(idObj, "get", key, 3600).then(({err, cache}) => {
					...
					// Если данные в кеше не найдены тогда продолжаем искать
						...
						const getData = function * (){
							...
							// Получаем регион
							region = yield idObj.getRegionByGPS({lat, lng});
							// Если регион найден
							if($.isset(region)) region = $.fnShowProps(region, "region");
							// Если регион найден
							if($.isset(region)){
								// Получаем район
								district = ($.isset(name) && $.isset(name.address)
				...
				Agl.getRedis.call(idObj, "get", key, 3600).then(({err, cache}) => {
					...
					// Если данные в кеше не найдены тогда продолжаем искать
						...
						const getData = function * (){
							...
							if($.isset(region)){
								// Получаем город
								city = ($.isset(name) && $.isset(name.address)
								...
								city = ($.isset(city) ? yield idObj.findCity({str: city, regionId: region._id}) : false);
								// Если это массив то извлекаем данные
				...
				Agl.getRedis.call(idObj, "get", key, 3600).then(({err, cache}) => {
					...
					// Если данные в кеше не найдены тогда продолжаем искать
						...
						const getData = function * (){
							...
							if($.isset(region)) region = $.fnShowProps(region, "region");
							// Получаем город
							city = yield idObj.getCityByGPS({lat, lng});
							// Если город найден
							if($.isset(city)) city = $.fnShowProps(city, "city");
							// Если город найден
							if($.isset(city)){
								// Получаем улицу
								street = ($.isset(name) && $.isset(name.address)
								...
								street = ($.isset(street) ? yield idObj.findStreet({str: street, cityId: city._id}) : false);
								// Если это массив то извлекаем данные
								if($.isArray(street) && street.length) street = yield compareGPS.call(idObj, lat, lng, street);
							}
							// Создаем объект для сохранения данных
				...
				info: () => {
					...
					if(this.debug.message){
						...
						console.info(
							...
							.anyks_clearColor(this.debug.color),
