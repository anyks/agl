			...
			return (new Promise(resolve => {
				...
				const reg = [{
					...
					// Регионы
						...
						"type":	"region",
						"reg":	new RegExp("(авт(?:ономный|\\.)\\s+окр?(?:уг|-г)?|область|край|республика|город)|"
								+ "(?:\\s|\\.|\\,|^)(респ?|ао(?:кр?)?|обл|кр|г)(?:\\s|\\.|\\,|$)", "i")
					// Районы
					},{
						"type":	"district",
						"reg":	new RegExp("(район|округ|улус|поселение)|(?:\\s|\\.|\\,|^)(р-н|окр|у|п)(?:\\s|\\.|\\,|$)", "i")
					// Города
					},{
						"type":	"city",
						"reg":	new RegExp("((?:пос[её]л(?:ение|ок|ки)\\s+(?:городского\\s+типа|сельского\\s+типа|и\\(при\\)\\s+станция\\(и\\)|"
					...
					// Улицы
						...
						"type":	"street",
						"reg":	new RegExp("(улица|площадь|переулок|гора|парк|тупик|канал|шоссе|проезд|набережная|километр|вал|бульвар|квартал|"
					...
					// Дома
						...
						"type":	"house",
						"reg":	/(дом|строение|корпус)|(?:\s|\.|\,|^)(дм?|стр|ко?рп?|ст\-е)(?:\s|\.|\,|$)/i
					// Дома
					},{
						"type":	"house",
