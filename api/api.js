	...
	class Agl {
		...
		parseAddress({address}){
			...
			return (new Promise(resolve => {
				...
				// Микрорайон
				const regCom = /(?:микрорайон|мкр\.)/i;
				// Города
				const regC = /(?:деревня|город|сельсовет|село|поселение|рабочий поселок|населенный пункт|поселок|посёлок|товарищество|станция|поселение городского типа|тов\.|нп\.|пгт\.|рп\.|д\.|г\.|с\.|ст\.|п\.|пос\.|пос-к\.|с/с\.|с-т\.|св-т\.|свт\.|св\.)/i;
				// Улицы
				...
				if(/\,/i.test(addObject.address)){
					...
					result = {
						...
						"river":		getAddress(regRi, "river"),
						"community":	getAddress(regCom, "community"),
						"house":		getHouse(),
						"zip":			getZip(),
						"country":		getCountry(),
