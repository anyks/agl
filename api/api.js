	...
	class Agl {
		...
		parseAddress({address}){
			...
			return (new Promise(resolve => {
				...
				+ "ж\\/д\\s+ст\\.))|(?:\\s|\\.|\\,|^)(с\\/?с|п\\/(?:о|ст)|ж\\/д(?:ст|(?:\\_|\\.)?\s*(?:рзд|пост))|с\\/(?:мо|п)|ст(?:-|\\.)\\s*ц?а|"
				+ "авто-а|кв-л|ма-в|р-?н|св?-т|за-ка|п(?:ромзона|ос-к|ст|гт|-к|к)|с(?:вх|мн|нт|вт|т|л|в)?|п(?:о?с)?|к(?:лх|п)?|т(?:ов|ер)|р(?:зд|п)|хз?|"
				+ "высел|зим|мкр|го|нп|вл|дп|м|д|у|г)(?:\\s|\\.|\\,|$)", "i");
				// Карта объектов
				const mapSubjects = {
					"р":		"Река",
					"оф":		"Офис",
