	...
	class Agl {
		...
		parseAddress({address}){
			...
			return (new Promise(resolve => {
				...
				const regRegion = new RegExp("(автономный\\s+округ|область|край|республика)|(?:(?:[\\s\\.\\,])(респ)(?:[\\s\\.\\,])"
				...
				const regCity = new RegExp("(пос[её]лок\\s+городского\\s+типа|рабочий\\s+пос[её]лок|курортный\\s+пос[её]лок|поселковый\\s+совет|"
				+ "сельский\\s+совет|дачный\\s+поселковый\\s+совет|городской\\s+округ|пос[её]лок\\s+сельского\\s+типа|насел[её]нный\\s+пункт|"
				+ "пос[её]лок\\s+при\\s+станции|железнодорожная\\s+станция|сельское\\s+поселение|коллективное\\s+хозяйство|"
				+ "садовое\\s+некоммерческое\\s+товарищество|советское\\s+хозяйство|поселение\\s+городского\\s+типа|дачный\\s+пос[её]лок|"
				+ "пос[её]лок|поселение|хозяйство|товарищество|зимовье|район|город|кишлак|поссовет|сельсовет|сомон|волость|село|местечко|"
				+ "деревня|слобода|станция|хутор|разъезд|колхоз)|(?:(?:[\\s\\.\\,])(пс)(?:[\\s\\.\\,])|(?:[\\s\\.\\,])(сс)(?:[\\s\\.\\,])|"
