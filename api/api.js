	...
	class Agl {
		...
		parseAddress({address}){
			...
			return (new Promise(resolve => {
				...
				const regDistrict = new RegExp("(район|округ|улус|поселение)|(?:(?:[\\s\\.\\,])(р-н)(?:[\\s\\.\\,])|"
				+ "(?:[\\s\\.\\,])(окр)(?:[\\s\\.\\,])|(?:[\\s\\.\\,])(у)(?:[\\s\\.\\,])|(?:[\\s\\.\\,])(п)(?:[\\s\\.\\,]))", "i");
				...
				+ "(?:выселк(?:и|ок)|заимка|аал|пос[её]лок|территория|хозяйство|товарищество|зимовье|район|кишлак|поссовет|сельсовет|сомон|волость|"
				+ "село|местечко|аул|станица|остров|автодорога|квартал|починок|жилрайон|массив|деревня|слобода|станция|хутор|разъезд|колхоз|улус|"
				+ "поселение|микрорайон|город(?:ок)?)|(?:п\\.г\\.т\\.|р\\.п\\.|к\\.п\\.|д\\.п\\.|н\\.п\\.|п\\.\\s+ст\\.|п\\.ст\\.|ж\\/д\\.\\s+ст\\.|"
				+ "ж\\/д\\s+ст\\.))|(?:\\s|\\.|\\,|^)(пс|сс|смн|р-н|у|г|снт|тер|дп|пос-к|мкр|го|св|св-т|свт|промзона|с-т|тов|пос|за-ка|зим|п-к|рзд|"
				+ "клх|ма-в|свх|пгт|х|ст|ж\\/д\_рзд|с\\/с|п\\/ст|п\\/о|сл|хз|рп|кп|высел|к|вл|п|ст-ца|ж\\/дст|ж\\/д\_пост|с\\/мо|нп|пст|авто-а|кв-л|"
				+ "с\\/п|с|м|д)(?:\\s|\\.|\\,|$)", "i");
