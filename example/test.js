...
const parseNativeAddress = address => {
	...
	const reg = [
		...
		{type: "zip", reg: /\d{6}/i},
		// Регионы
		{type: "region", reg: new RegExp("(авт(?:ономный|\\.)\\s+окр?(?:уг|-г)?|область|край|республика|город)|"
		+ "(?:\\s|\\.|\\,|^)(респ?|ао(?:кр?)?|обл|кр|г)(?:\\s|\\.|\\,|$)", "i")},
		// Районы
		{type: "district", reg: new RegExp("(район|округ|улус|поселение)|(?:\\s|\\.|\\,|^)(р-н|окр|у|п)(?:\\s|\\.|\\,|$)", "i")},
		// Города
		{type: "city", reg: new RegExp("((?:пос[её]л(?:ение|ок|ки)\\s+(?:городского\\s+типа|сельского\\s+типа|и\\(при\\)\\s+станция\\(и\\)|"
		...
		// Улицы
		{type: "street", reg: new RegExp("(улица|площадь|переулок|гора|парк|тупик|канал|шоссе|проезд|набережная|километр|вал|бульвар|квартал|"
		+ "проспект|авеню|аллея|кольцо)|(?:\\s|\\.|\\,|^)(ул|пл|пр-?к?т?|ав|алл?|б-?р|вл|кнл|кв-л|к(?:м|л)|клц|на?б|пер|пр-зд|туп|ш|гор)(?:\\s|\\.|\\,|$)", "i")},
		// Микрорайоны
		{type: "community", reg: /(микрорайон|жилой\s+комплекс)|(?:\s|\.|\,|^)(мкр|жкс?)(?:\s|\.|\,|$)/i},
		// Квартиры
		{type: "apartment", reg: /(квартира|офис|комната)|(?:\s|\.|\,|^)(кв|ко?м|оф)(?:\s|\.|\,|$)/i},
		// Дома
		{type: "house", reg: /(дом|строение|корпус)|(?:\s|\.|\,|^)(дм?|стр|ко?рп?|ст\-е)(?:\s|\.|\,|$)/i},
		// Дома
		{type: "house", reg: new RegExp("(?:(?:№\\s*)?\\d+[А-ЯЁ]*\\s*(?:\\/|-)\\s*\\d+[А-ЯЁ]*)|"
		+ "(?:(?:№\\s*)?(?:\\d+)[А-ЯЁ]*\\s*(?:к|с)?\\s*(?:\\d+)?\\s*(?:к|с)?\\s*(?:\\d+)?)$", "i")},
		// Реки
		{type: "river", reg: /(река)|(?:^|\s)(р(?:-ка)?)(?:\s|\.|\,|$)/i},
		// Страны
		{type: "country", reg: /(страна)|(?:\s|\.|\,|^)(стр?-?н?а?)(?:\s|\.|\,|$)/i}
	];
	// Карта объектов
	const mapSubjects = {
		"р":			"Река",
